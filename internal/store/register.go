...
func (s *Scope) RegisterInstance(
	...
	if err := tx.QueryRow(ctx, `
		INSERT INTO schemaver.credential
		    (name, username, kind, secret_ciphertext, key_id, project_id)
		VALUES ($1, $2, 'inline', $3, $4, $5)
		RETURNING id`,
		name+" credential", username, ciphertext, s.store.box.KeyID(),
		s.writable).Scan(&credentialID); err != nil {
		return 0, fmt.Errorf("store credential: %w", err)
	}

	var instanceID int64
	if err := tx.QueryRow(ctx, `
		INSERT INTO schemaver.instance (name, host, port, tls_mode, credential_id, project_id)
		VALUES ($1, $2, $3, $4, $5, $6)
		RETURNING id`,
		name, host, port, tlsMode, credentialID, s.writable).Scan(&instanceID); err != nil {
...
func (s *Scope) SetManaged(ctx context.Context, instanceID int64, names []string) (int, error) {
	...
	if len(names) == 0 {
		tag, err = s.store.pool.Exec(ctx, `
			...
			instanceID, s.projects)
	} else {
		tag, err = s.store.pool.Exec(ctx, `
			UPDATE schemaver.database SET managed = true
			 WHERE instance_id = $1 AND archived_at IS NULL AND name = ANY($2)
			   AND instance_id IN (SELECT id FROM schemaver.instance WHERE project_id = ANY($3))`,
			instanceID, names, s.projects)
	}
	if err != nil {
...
func (s *Scope) PairForDrift(ctx context.Context, databaseID, peerID int64) error {
	...
	if _, err := s.store.pool.Exec(ctx, `
		UPDATE schemaver.database d
		 ...
		 WHERE d.id = $1 AND d.instance_id = i.id AND i.project_id = ANY($3)
		   AND pd.id = $2 AND pd.instance_id = p.id AND p.project_id = ANY($3)`,
		databaseID, peerID, s.projects); err != nil {
...
func (s *Scope) DatabaseIDByName(ctx context.Context, instanceID int64, name string) (int64, error) {
	...
	if err := s.store.pool.QueryRow(ctx, `
		SELECT d.id FROM schemaver.database d
		 ...
		 WHERE d.instance_id = $1 AND d.name = $2 AND i.project_id = ANY($3)`,
		instanceID, name, s.projects).Scan(&id); err != nil {
