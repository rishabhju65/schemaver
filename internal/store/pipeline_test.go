...
// TestAnAdoptedDatabaseIsTaggedProduction covers the default a database arrives
// with, which moved when discovery stopped creating rows.
//
...
// that check permissive because there are no ranks to compare. Leaving it unset
// quietly disables a safeguard.
//
...
// through anything.
func TestAnAdoptedDatabaseIsTaggedProduction(t *testing.T) {
	...
	// Adopted in bulk, where nobody is asked about each one.
	if _, err := scope.AdoptDatabases(ctx, userID, instanceID, 0, []string{"unlabelled"}); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		SELECT e.name, e.rank FROM schemaver.database d
		 ...
		 WHERE d.instance_id = $1 AND d.name = 'unlabelled'`, instanceID).
		Scan(&name, &rank); err != nil {
		t.Fatalf("the adopted database has no environment at all: %v", err)
	}

	var highest int
	pool.QueryRow(ctx, `
		SELECT max(rank) FROM schemaver.environment WHERE project_id = $1`,
		projectID).Scan(&highest)
	if rank != highest {
		t.Errorf("adopted as %s (rank %d); the most guarded is rank %d",
			name, rank, highest)
	}
}
